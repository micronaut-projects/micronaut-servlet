 ...
 */
class UndertowGracefulShutdownSpec extends Specification {

    void "the server is graceful shutdown capable and reports in-flight requests"() {
        given:
        EmbeddedServer server = ApplicationContext.run(EmbeddedServer, ['spec.name': 'UndertowGracefulShutdownSpec'])
    ...
    void "stopping the application waits for an in-flight request to complete"() {
        ...
        EmbeddedServer server = ApplicationContext.run(EmbeddedServer, [
            'spec.name'                                   : 'UndertowGracefulShutdownSpec',
    ...
    @Requires(property = 'spec.name', value = 'UndertowGracefulShutdownSpec')
    @Controller('/graceful')
