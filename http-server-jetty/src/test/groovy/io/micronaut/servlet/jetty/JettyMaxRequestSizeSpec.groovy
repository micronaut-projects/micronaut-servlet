 ...
 */
@MicronautTest
@Property(name = 'spec.name', value = 'JettyMaxRequestSizeSpec')
@Property(name = 'micronaut.server.max-request-size', value = '1024')
class JettyMaxRequestSizeSpec extends Specification {
    ...
    void "a form submission that declares no length is refused with 413 once it grows past the limit"() {
        given: "the container would otherwise parse the chunked form straight from the stream, subject only to its own limit"
        SizeController controller = embeddedServer.applicationContext.getBean(SizeController)
        controller.reached = false
        String form = 'name=' + ('x' * 4096)
        ...
        HttpResponse<String> response = client.send(HttpRequest.newBuilder(embeddedServer.URI.resolve('/size/form'))
            .header('Content-Type', MediaType.APPLICATION_FORM_URLENCODED)
            .POST(HttpRequest.BodyPublishers.fromPublisher(HttpRequest.BodyPublishers.ofString(form)))
            .build(), HttpResponse.BodyHandlers.ofString())
    ...
    void "a form submission that declares no length still binds its fields and the query parameters"() {
        given: "a field with an encoded value, a repeated field, and a query parameter alongside"
        String form = 'name=a+b%26c&tag=one&tag=two'

        when:
        HttpResponse<String> response = client.send(HttpRequest.newBuilder(embeddedServer.URI.resolve('/size/form-fields?source=query'))
            .header('Content-Type', MediaType.APPLICATION_FORM_URLENCODED)
            .POST(HttpRequest.BodyPublishers.fromPublisher(HttpRequest.BodyPublishers.ofString(form)))
            .build(), HttpResponse.BodyHandlers.ofString())
        ...
        then:
        response.statusCode() == 200
        response.body() == 'name=a b&c tags=[one, two] source=query'
    }
    ...
    @Requires(property = 'spec.name', value = 'JettyMaxRequestSizeSpec')
    @Controller('/size')
    static class SizeController {
        ...
        @Post('/form-fields')
        @Consumes(MediaType.APPLICATION_FORM_URLENCODED)
        String formFields(String name, List<String> tag, @QueryValue String source) {
            "name=$name tags=$tag source=$source"
        }
    }
}
