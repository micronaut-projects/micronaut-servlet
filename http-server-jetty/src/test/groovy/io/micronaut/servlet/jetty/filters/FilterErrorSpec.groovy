...
class FilterErrorSpec extends Specification {
    ...
    void "test errors emitted from first filter interacting with exception handlers"() {
        EmbeddedServer server = ApplicationContext.run(EmbeddedServer, [
                'spec.name': FilterErrorSpec.simpleName,
                'micronaut.http.client.read-timeout': '1h'
        ])
    ...
    void "test errors emitted from second filter interacting with exception handlers"() {
        ...
        when:
        HttpResponse<String> response = Flux.from(client.exchange(HttpRequest.GET("/filter-error-spec").header("X-Passthru", "true"), String))
                .onErrorResume(t -> {
                    if (t instanceof HttpClientResponseException) {
                        return Flux.just(((HttpClientResponseException) t).response)
                    }
...
//        def firstResponse = first.response.getAndSet(null)
        def firstResponseStatus = first.responseStatus.getAndSet(null)
...
//        firstResponse.status() == HttpStatus.BAD_REQUEST
        firstResponseStatus == HttpStatus.BAD_REQUEST
...
//        def filterResponse = filter.response.getAndSet(null)
        def filterResponseStatus = filter.responseStatus.getAndSet(null)
