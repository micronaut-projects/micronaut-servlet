 ...
 */
class JettyGracefulShutdownSpec extends Specification {

    void "the server is graceful shutdown capable and reports in-flight requests"() {
        given:
        EmbeddedServer server = ApplicationContext.run(EmbeddedServer, ['spec.name': 'JettyGracefulShutdownSpec'])
    ...
    void "stopping the application waits for an in-flight request to complete"() {
        ...
        EmbeddedServer server = ApplicationContext.run(EmbeddedServer, [
            'spec.name'                                   : 'JettyGracefulShutdownSpec',
    ...
    @Requires(property = 'spec.name', value = 'JettyGracefulShutdownSpec')
    @Controller('/graceful')
