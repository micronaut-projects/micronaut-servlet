...
class JettySessionSpec extends Specification {

    @Shared
    @AutoCleanup
    EmbeddedServer embeddedServer = ApplicationContext.run(EmbeddedServer, ['spec.name': 'JettySessionSpec'])
    ...
    @Requires(property = 'spec.name', value = 'JettySessionSpec')
    @Client('/session')
