 ...
 */
@MicronautTest
@Property(name = 'spec.name', value = 'JettyFileResponseSpec')
class JettyFileResponseSpec extends Specification {

    static final String CONTENT = 'the quick brown fox jumps over the lazy dog'

    @Shared
    @AutoCleanup('deleteDir')
    static Path directory = Files.createTempDirectory('JettyFileResponseSpec')
    ...
    @Requires(property = 'spec.name', value = 'JettyFileResponseSpec')
    @Controller('/files')
