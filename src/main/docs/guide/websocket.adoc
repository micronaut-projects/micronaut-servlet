...
to the value an `@OnMessage` method returns; the type an encoder handles is read from its
bean definition, or from its generic signature when `micronaut-reflection` is present, and
an encoder whose type is unknown is offered every value and stands aside for one it cannot
encode. Declaring encoders as beans is therefore the simplest choice. `RemoteEndpoint#sendObject` on the raw session
