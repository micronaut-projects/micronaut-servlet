            ...
            if (bodyType != null) {
                ...
                if (CharSequence.class.isAssignableFrom(bodyJavaType)) {
                    try (BufferedReader reader = serverlessHttpRequest.getReader()) {
                        ...
                        return () -> Optional.of(
                                new ServletRequestAndBody(serverlessHttpRequest, content)
                        );
                    } catch (IOException e) {
                        return () -> Optional.of(
                                new ServletRequestAndBody(serverlessHttpRequest, "")
                        );
                    }
                } else {
                    ...
                    if (codec != null) {
                        try (InputStream reader = serverlessHttpRequest.getInputStream()) {
                            ...
                            return () -> Optional.of(
                                    new ServletRequestAndBody(serverlessHttpRequest, result)
                            );
                        } catch (IOException | CodecException e) {
                            if (ServletHttpHandler.LOG.isDebugEnabled()) {
                                ServletHttpHandler.LOG.debug("Error decoding media type: " + mediaType, e);
                            }
