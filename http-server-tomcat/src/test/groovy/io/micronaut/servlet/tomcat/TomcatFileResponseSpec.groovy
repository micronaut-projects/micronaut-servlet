 ...
 */
@MicronautTest
@Property(name = 'spec.name', value = 'TomcatFileResponseSpec')
class TomcatFileResponseSpec extends Specification {

    static final String CONTENT = 'the quick brown fox jumps over the lazy dog'

    @Shared
    @AutoCleanup('deleteDir')
    static Path directory = Files.createTempDirectory('TomcatFileResponseSpec')
    ...
    @Requires(property = 'spec.name', value = 'TomcatFileResponseSpec')
    @Controller('/files')
